...
learning_rates = 0.05#,0.1,0.05,0.02,0.01,0.005,0.002,0.001,0.0005]
    ...
    for run_no in range(len(alphas)):
        ...
        for i,data in enumerate(train_loader):
            with torch.no_grad():
                ...
                alpha_weights = torch.ones((batch,patches,1),dtype = torch.float32,
                                           device=device)*((1-alphas[run_no])/(patches-1))
                

                alpha_weights[:,0] = torch.ones((batch,1),dtype=torch.float32,
                                                device=device)*alphas[run_no]
                ...
                alpha_weights_ = torch.ones((batch,patches,1),dtype = torch.float32,
                                            device=device)*((1-alpha_)/(patches-1))
                
                
                alpha_weights_[:,0] = torch.ones((batch,1),dtype = torch.float32,
                                                 device=device)*alpha_
...
plt.savefig("sa_m_"+str(m)+"_c_"+str(n_classes)+".pdf",bbox_inches='tight')
plt.show()
