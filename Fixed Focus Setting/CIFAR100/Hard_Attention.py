...
def labels_to_index(fg_labels):
    unique_foreground_labels = list(np.unique(foreground_label))
    foreground_labels = []
    for fg in fg_labels:
...
foreground_label = labels_to_index(foreground_label)
...
for i in tqdm(range(desired_num)):
    set_idx = set()
    np.random.seed(i)
    bg_idx = np.random.randint(0,25000,m-1)
    set_idx = set(background_label[bg_idx].tolist())
    fg_idx = np.random.randint(0,25000)
    ...
    for run_no in range(len(alphas)):
        ...
        for i,data in enumerate(train_loader):
            with torch.no_grad():
                ...
                alpha_weights = torch.ones((batch,patches),device=device)*((1-alphas[run_no])/(patches-1))
                alpha_weights[:,0] = torch.ones(batch,device=device)*alphas[run_no]
                ...
                alpha_weights_ = torch.ones((batch,patches),device=device)*((1-alpha_)/(patches-1))
                alpha_weights_[:,0] = torch.ones(batch,device=device)*alpha_
