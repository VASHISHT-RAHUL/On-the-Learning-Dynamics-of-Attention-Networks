...
train_sentences[1], train_rationales[1],train_labels[1]




torch.unique(torch.hstack(train_labels))
...
with torch.no_grad():
    for i,data in enumerate(processedtrainloader):
        ...
        outputs1 = torch.sum(alpha[:,:,None]*outputs,dim=1)
...
run_no = 0
learning_rates =[0.3] #[0.2,0.1,0.05,0.02,0.01,0.005,0.002,0.001,0.0005]import wandb
